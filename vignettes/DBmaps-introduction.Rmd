...
*   [**customers.csv**](https://github.com/akshat09867/Retail-data/blob/main/inst/extdata/customers.csv)
*   [**products.csv**](https://github.com/akshat09867/Retail-data/blob/main/inst/extdata/products.csv)
*   [**transactions.csv**](https://github.com/akshat09867/Retail-data/blob/main/inst/extdata/transactions.csv)
*   [**views.csv**](https://github.com/akshat09867/Retail-data/blob/main/inst/extdata/views_sample.csv)

First, let's load these tables into our R session. For this example, we will assume the files have been downloaded to a local `extdata` directory.
...
customers <- data.table::fread(path_customers)
products <- data.table::fread(path_products)
transactions <- data.table::fread(path_transactions)
views <- data.table::fread(path_views)
...
print(plan)
```
The planner has correctly identified that it needs to perform two separate aggregations (one on `transactions` and one on `views`) and then chain two merge 
operations to combine them with the `products` table.
